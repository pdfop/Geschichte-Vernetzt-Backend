...
def generate_report():
    """
        generates a report of user answers to questions in tours and returns it as html page
        parameters are passed by the URL
        parameters:
            type : String, decides which type of report to generate, either "user", "me" or "question"
            tour: String, object id of the tour associated with the question(s)
        "me" reports can be generated by any member of the tour
        "question" reports can only be generated by the tour owner and admins
            they expect an additional parameter
                question : String, object id of the question to get user answers for
        "user" reports can only be generated by the tour owner and admins
            they expect an additional parameter
                user : String, username of the user to generate the report for
        all calls expect a jwt access token, passed in an 'Authorization' header with the value "Bearer <token>"
            the token must not include the quotation marks i.e. Bearer ktifkd NOT Bearer "ktifkd"
        returns a html report if successful
        returns status code 404 if any parameters where invalid or token was not authorized for "user" or "question" type
    """
